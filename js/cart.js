import { catalogList, modalProductBtn, countAmount, orderCount, orderList, orderTotalAmount, orderWrapTitle, order, orderSubmit, modalDelivery } from "./elements.js";
    ...
    const cartItems = data.map(item => {
        ...
        li.innerHTML = `
          ...
          <div class="order__product">
              ...
              <p class="order__product-value">
                  <span class="order__product-price">${item.price}</span>
...
const cartController = () => {
    ...
    modalDelivery.addEventListener('click', ({target}) => {
        if (target.closest('.modal__close') || modalDelivery === target) {
            modalDelivery.classList.remove('modal__open');
        }
    });
};
